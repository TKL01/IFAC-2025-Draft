...
\begin{abstract}                % Abstract of not more than 250 
	In this work, deep neural networks made up of multiple hidden Long Short-Term Memory (LSTM) and Feed Forward layers are trained to predict the  thermal behavior of the joint motors of robot manipulators. A model-free approach is adopted. It allows for the accommodation of  complexity and uncertainty challenges that might compromise the identification and validation of a usually large number of  parameters of an approximation model hardly available. By contrast, sensed joint torques are  collected  and processed using the machine learning approach to foresee the thermal behavior of joint motors. Prediction results related to the thermal dynamics of joint motors of a redundant robot with seven joints are presented. 
\end{abstract}
