...
Robot are used to achieve repetitive tasks and assist humans in many applications. Large joint accelerations, high payload manipulation, and specific configurations belong to those factors that can induced an overheating of their joint motors. Excessive motor temperature might jeopardize the 

Most robot manufacturers, including Franka, Kinova, and KUKA, offer built-in function to shut down the robot one a critical temperature threshold is attained. Whereas this approx helps prevent a deterioration of the motor, it is likely to compromise the robot availability for production purposes. Furthermore, thermal burns in physical human -robot-interaction not only represents a sever safety issue but also the user experience necessary to engage a symbiotic interaction.

It is therefore important to address motor overheating to sustain the availability,  prolong the lifetime, and 
