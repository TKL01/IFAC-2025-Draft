...
The fulls state of our experimental Kinova Gen 3 ultra-lightweight robot with seven degrees-of-freedom is recorded using a specially developed OPC Unified Architecture server (\cite{Girke}). 
...
Completed experiments indicate that the second and fourth experience the highest temperature increase. Collected datasets are freely available on our streamlit web app, 
...
Feedforward neural networks (FNN) shift data from the input to the output of a neural network. Backpropagation is run to optimize the neural weights. FNN have a simplistic strcuture and are limited in their ability to incorporate past values. 
...
\begin{itemize}
  ...
  \item FNNs can be trained with lower computational power and in a shorter time compared to LSTMs, which consists of more complex structures (e.g., sates and gates). LSTMs store and update past information.
  \item Joint temperatures change due to continous loads as well as temporal dynamics of joint torques. The time-dependency can be captures by LSTMs. They are specifically designed for time series. FNNs are more suitable for regression problems on static data points where a disturbance or reordering of the temporal dynamics is non-critical.
\end{itemize}
% \begin{table}[hb]
%   \begin{center}
